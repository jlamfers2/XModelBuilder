...
/// <summary>
/// A small, dependency-free <see cref="IFaker"/> with deterministic primitives that Bogus
/// deliberately does NOT do well: monotonic identity counters, stable name-based GUIDs and
/// clock-bound ages. Meant to live ALONGSIDE a rich data faker (e.g. XModelBuilder.Bogus), not
/// to replace it.
///
/// <para>
/// All randomness flows through the injected, seeded <see cref="Random"/>; all "now"-relative
/// values flow through the injected <see cref="TimeProvider"/>. Register it with
/// <c>AddXFaker(seed)</c>. The determinism/isolation boundary is the ServiceProvider: build a
/// fresh one per test (each with the same seed) and every run reproduces - separate providers get
/// separate seeded RNGs and counters that start over, so parallel tests stay safe.
/// </para>
///
/// <para>
